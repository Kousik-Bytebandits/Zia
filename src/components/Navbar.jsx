...
useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      try {
        const response = await fetch(
          `https://api.ziaherbalpro.com/Microservices/04_userProducts/api/suggestions?q=${encodeURIComponent(searchTerm)}`
        );
  ...
  return (
    <>
      <nav className="flex flex-col lg:flex-row text-white w-full ">
        ...
        <div
          className={`w-full lg:hidden px-4 py-3 pb-3 z-50 transition-all duration-300 fixed top-0 ${isScrolled ? 'bg-[#2f3a27] shadow-md' : 'bg-[#2f3a27]'}`}
        >
          {!isScrolled ? (
            <>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <img src="/images/zia_logo.webp" alt="Zia Logo" className="h-11" />
                </div>
  ...
  {showDropdown && suggestions.length > 0 && (
    <ul className="absolute top-full left-0 w-full z-50 bg-white border border-gray-300 rounded-md shadow-md max-h-60 overflow-y-auto">
      {suggestions.map((suggestion, index) => (
        <li
          key={suggestion.product_id || index}
          className="cursor-pointer hover:bg-gray-100 px-4 py-2 text-black"
          onMouseDown={() => {
            navigate(`/shopdetails/${suggestion.product_id}`);
            setSearchTerm("");
            setShowDropdown(false);
            setSuggestions([]);
          }}
      ...
      {suggestions.map((suggestion, index) => (
        ...
        <div className="hidden laptop:flex w-full fixed top-0 z-50 font-archivo ">
          <div className="bg-[#2f3a27] flex items-center px-4 laptop:px-4 xxxl:px-10 py-3 gap-6 hd:gap-10 laptop:gap-6 xxxl:gap-14 laptop:w-[45%] xxxl:w-[45%] ">
            <img src="/images/zia_logo.webp" alt="Zia Logo" className="h-12 laptop:h-14 xxxl:h-16 px-2 shrink-0" />
            <ul className="flex gap-6 laptop:gap-6 xxxl:gap-14 hd:gap-10 hd:text-[18px] text-[16px] laptop:text-[17px] xxxl:text-[20px] items-center justify-start ">
              ...
              <NavLink to="/contactus" className={({ isActive }) => `${isActive ? 'border-b-2 border-white' : ''}`}>Contact Us</NavLink>
              <NavLink to="/order-tracking" className={({ isActive }) => `${isActive ? 'border-b-2 border-white' : ''}`}>Track Order</NavLink>
            </ul>
          </div>
  ...
  <ul className="absolute z-50 top-full  w-[100%]  bg-white border border-gray-200  rounded shadow ">
    {suggestions.map((suggestion, index) => (
  <li
    key={suggestion.product_id || index}
    className="cursor-pointer hover:bg-gray-100 px-4 py-3 text-[18px] text-black"
    onMouseDown={() => {
      navigate(`/shopdetails/${suggestion.product_id}`);
      setSearchTerm("");
      setShowDropdown(false);
  ...
  >
    {suggestion.name}
      ...
      {/* Mobile Slide Menu */}
      {isOpen && <div className="fixed inset-0 z-40 bg-black bg-opacity-40" onClick={() => setIsOpen(false)} />}
      <div
        className={`fixed top-0 left-0 h-full w-64 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out z-50 shadow-xl`}
      ...
      >
        ...
        <div className="h-[calc(100%-64px)] p-4 text-lg text-left text-[#2B452C]" style={{ backgroundColor: '#93A88DF2' }}>
